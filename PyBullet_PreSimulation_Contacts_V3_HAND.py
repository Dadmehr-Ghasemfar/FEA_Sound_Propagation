...
p.removeBody(handID)
# scaling_factor = float(vars.obj_scaling_factor)/float(4.71387499)
scaling_factor = float(vars.obj_scaling_factor)/float(max(dim_bounding_box))
scaling_factor = 0.21213969422218354
# FIX THIS
boxStartPos = [-0.80875437*scaling_factor, 1.02478381*scaling_factor, 0.17250621*scaling_factor]
...
boxID = p.createMultiBody(baseMass=vars.big_M,
                      ...
                      baseCollisionShapeIndex=collisionShapeId,
                      baseVisualShapeIndex=visualShapeId,
                      basePosition=[-0.2, 0, vars.init_height],
                      useMaximalCoordinates=True)

p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1)
p.stopStateLogging(logId)
p.setGravity(0, 0, vars.grav_acc)
p.resetDebugVisualizerCamera(cameraDistance = vars.camera_dist/2, cameraYaw=-90, cameraPitch=-45, cameraTargetPosition=[-0.80875437*scaling_factor, 1.02478381*scaling_factor, 0.17250621*scaling_factor])
